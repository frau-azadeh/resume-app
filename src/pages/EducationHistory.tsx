...
const EducationHistory: React.FC = () => {
  ...
  const handleEdit = (index: number) => {
    ...
    Object.keys(item).forEach((key) => {
      setValue(
        key as keyof EducationFormData,
        item[key as keyof EducationFormData],
      );
    });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow" dir="rtl">
      ...
      >
        ...
        <Input
          label="رشته"
          {...register("field", { required: "رشته الزامی است" })}
        />
        <Input
          label="گرایش"
          {...register("specialization", { required: "گرایش الزامی است" })}
        />
        <Input
          label="نوع آموزشگاه/دانشگاه"
          {...register("institutionType", { required: true })}
        />
        <Input
          label="نام آموزشگاه/دانشگاه"
      ...
      {educationList.length === 0 && (
        <p className="text-gray-500">هنوز سابقه‌ای ثبت نشده است.</p>
      )}
          ...
          >
            <div>
              <p className="font-semibold">
                {edu.degree} - {edu.field} ({edu.institutionName})
              </p>
              <p className="text-sm text-gray-600">
                {edu.startDate} تا{" "}
                {edu.isStudying ? "در حال تحصیل" : edu.endDate}
              </p>
            </div>
