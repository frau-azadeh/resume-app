...
const PersonalInfo: React.FC = () => {
  ...
  return (
    <Layout>
      <div className="max-w-md mx-auto p-6 space-y-4 bg-white rounded-lg shadow" dir="rtl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Button type="submit" className="w-full">ثبت اطلاعات</Button>
        </form>

        {savedInfo.firstName && (
          <div className="mt-6 p-4 border rounded-md bg-gray-50">
            <h2 className="text-lg font-semibold mb-2">اطلاعات ذخیره شده:</h2>
            <p><strong>نام:</strong> {savedInfo.firstName}</p>
            <p><strong>نام خانوادگی:</strong> {savedInfo.lastName}</p>
            <p><strong>ایمیل:</strong> {savedInfo.email}</p>
            <p><strong>شماره تماس:</strong> {savedInfo.phone}</p>
            <p><strong>آدرس:</strong> {savedInfo.address}</p>
          </div>
        )}
      </div>
    </Layout>
