...
const EducationForm: React.FC<Props> = ({ initialData, onSubmit, onCancel, isEditing }) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      {errors.degree && <p className="text-red-600 text-sm">{errors.degree.message}</p>}
      ...
      <div className="col-span-2 text-center flex gap-4 justify-center">
        <Button type="submit">{isEditing ? "ویرایش سابقه" : "ثبت سابقه"}</Button>
        {isEditing && (
