...
const EducationList: React.FC<Props> = ({ educationList, onEdit, onDelete }) => (
  <div className="mt-6">
      ...
      >
        <div>
          ...
          <div>
            ...
            تا{" "}
            {item.isStudying ? "در حال تحصیل" : item.endDate ? dayjs(item.endDate).format("YYYY-MM-DD") : ""}
          </div>
        </div>
        <div className="flex gap-2">
          <button onClick={() => onEdit(index)} className="text-blue-600">ویرایش</button>
          <button onClick={() => onDelete(index)} className="text-red-600">حذف</button>
        </div>
      </div>
    ))}
  </div>
