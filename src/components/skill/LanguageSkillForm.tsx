...
const LanguageSkillForm: React.FC<Props> = ({ onAdd, languageSkills, onDelete }) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  return (
    <div dir="rtl" className="mx-auto bg-white p-6 rounded-lg shadow-md space-y-8 mb-10">
            <h3 className=" font-semibold mb-4 text-gray-800 text-right">مهارت‌های زبان خارجی</h3>
        ...
        {["reading", "writing", "speaking", "comprehension"].map((field) => (
          <div key={field}>
            ...
            <select
              ...
              className={`w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400 ${
                errors[field as keyof LanguageSkillData] ? "border-red-500" : "border-gray-300"
              }`}
              defaultValue=""
            >
